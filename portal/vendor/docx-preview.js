            ...
            for (var n of globalXmlParser.elements(node)) {
                switch (n.localName) {
                    ...
                    case "wrapNone":
                        ...
                        break;
                    case "docPr":
                        // DOC-P2D-02: expose the drawing anchor (wp:docPr id) so in-place image
                        // replacement can target the matching <w:drawing> in source.docx.
                        result.docPrId = globalXmlParser.attr(n, "id");
                        break;
                    case "graphic":
...
`;
        ...
        renderParagraph(elem) {
            ...
            if (elem.paraId)
                result.setAttribute("data-el", elem.paraId);
        ...
        renderDrawing(elem) {
            ...
            if (elem.docPrId != null && elem.docPrId !== "")
                result.setAttribute("data-doc-pr-id", elem.docPrId);   // DOC-P2D-02 anchor
            this.renderChildren(elem, result);
            this.renderStyleValues(elem.cssStyle, result);
            return result;
        }
